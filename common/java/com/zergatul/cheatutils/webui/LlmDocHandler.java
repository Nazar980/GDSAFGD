...
public class LlmDocHandler implements HttpHandler {
    ...
    private List<String> generateLines() {
        ...
        lines.add("// ==========================================");
        lines.add("// Java Scripting Language - API Definitions");
        ...
        for (SType type : apis.stream().map(PropertyReference::getType).sorted(Comparator.comparing(this::formatType)).toList()) {
            lines.add("");
            buildType(lines, type);
        }
    ...
    private void buildType(List<String> lines, SType type) {
    ...
    private String formatType(SType type) {
    ...
    private List<SType> getPrimitiveTypes() {
        return List.of(
                SBoolean.instance,
                SInt8.instance,
                SInt16.instance,
